<template>
  <div class="h-[calc(100vh-7rem)] flex flex-col md:flex-row gap-4 md:gap-6 p-2 md:p-4 max-w-7xl mx-auto relative">
    ...
    <div class="md:hidden flex-shrink-0 flex justify-between items-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-md p-3 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm z-10">
      <div class="min-w-0 flex-1 mr-2">
        <h2 class="font-bold text-gray-900 dark:text-white truncate">{{ room?.name || 'Chat Room' }}</h2>
      ...
      <button 
        ...
        class="p-2 text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors"
      >
    ...
    <div 
      ...
      :class="[
        mobileSidebarOpen ? 'fixed inset-0 z-50 bg-gray-50/95 dark:bg-gray-900/95 backdrop-blur-sm p-4 overflow-y-auto' : 'hidden md:flex w-full md:w-80'
      ]"
    >
      <!-- Mobile Close Button -->
      <div class="md:hidden flex justify-end mb-2">
        <button 
          ...
          class="p-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white bg-white dark:bg-gray-800 rounded-full shadow-sm"
        >
      ...
      <!-- Room Info Card -->
      <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-sm border border-gray-100 dark:border-gray-700 rounded-2xl p-5 transition-all hover:shadow-md">
        <div v-if="loadingRoom" class="text-center py-4 text-gray-400 animate-pulse">Loading room...</div>
        <template v-else-if="room">
          <div class="mb-4">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white tracking-tight">{{ room.name }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 font-medium">{{ room.topic }}</p>
          </div>

          <div class="flex items-center justify-between mb-6 bg-gray-50/50 dark:bg-gray-700/50 p-3 rounded-xl border border-gray-100 dark:border-gray-600">
            <div class="flex items-center gap-2">
              ...
              <span class="text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-300">{{ room.status }}</span>
            </div>
            <span class="text-xs font-mono text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-800 px-2 py-1 rounded-md shadow-sm border border-gray-100 dark:border-gray-600">
          ...
          <div class="space-y-3">
            ...
            <button
              ...
              @click="finishChat"
              :disabled="finishing"
              class="group w-full flex items-center justify-center py-2.5 px-4 rounded-xl shadow-sm text-sm font-medium text-white bg-gray-600 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 disabled:opacity-50 transition-all active:scale-95 mt-2"
            >
              <span v-if="finishing" class="mr-2">
                <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                  <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
              </span>
              {{ finishing ? 'Terminating...' : 'Terminate Chat' }}
            </button>
            ...
            <div class="grid grid-cols-2 gap-3 pt-2">
              <button
                ...
                class="flex items-center justify-center py-2 px-3 border border-gray-200 dark:border-gray-600 rounded-xl shadow-sm text-xs font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-500 focus:outline-none transition-colors"
              >
                Restart
              </button>
              <button
                @click="deleteRoom"
                :disabled="deleting"
                class="flex items-center justify-center py-2 px-3 border border-gray-200 dark:border-gray-600 rounded-xl shadow-sm text-xs font-medium text-red-600 dark:text-red-400 bg-white dark:bg-gray-800 hover:bg-red-50 dark:hover:bg-red-900/20 hover:border-red-200 dark:hover:border-red-800 focus:outline-none transition-colors"
              >
                Delete
              </button>
            </div>
            ...
            <button
              @click="router.push('/')"
              class="w-full flex items-center justify-center py-2 px-4 rounded-xl text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors"
            >
              Back to Dashboard
            </button>
          </div>
      ...
      <!-- Roles List -->
      <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-md shadow-sm border border-gray-100 dark:border-gray-700 rounded-2xl p-5 flex-1 overflow-hidden flex flex-col min-h-[200px]">
        ...
        <div class="overflow-y-auto pr-2 -mr-2 space-y-1 custom-scrollbar">
          <div v-if="room" v-for="role in room.roles" :key="role.id" class="group flex items-center p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-default">
            <div class="relative">
              <div class="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold shadow-sm ring-2 ring-white dark:ring-gray-600">
                {{ role.name.charAt(0) }}
              </div>
              <div class="absolute -bottom-1 -right-1 bg-white dark:bg-gray-800 rounded-full p-0.5" v-if="activeRoleId === role.id">
                 <div class="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse"></div>
              </div>
            </div>
            <div class="ml-3 min-w-0">
              <p class="text-sm font-semibold text-gray-900 dark:text-white truncate">{{ role.name }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{{ role.profession || 'Participant' }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Main Chat Area -->
    <div class="flex-1 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm shadow-sm border border-gray-100 dark:border-gray-700 rounded-2xl flex flex-col h-full overflow-hidden relative">
      <!-- Chat Header (optional context) -->
      <div class="absolute top-0 left-0 right-0 h-6 bg-gradient-to-b from-white/90 dark:from-gray-800/90 to-transparent z-10 pointer-events-none"></div>

      <div class="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar scroll-smooth" ref="chatContainer">
        <div v-if="loadingMessages" class="flex flex-col items-center justify-center h-full text-gray-400 dark:text-gray-500 space-y-3">
        ...
        <div v-else-if="messages.length === 0" class="flex flex-col items-center justify-center h-full text-gray-400 dark:text-gray-500">
        ...
        <transition-group name="message-fade">
          <div v-for="msg in messages" :key="msg.id" class="flex flex-col">
            ...
            <div v-if="!msg.role_id && !msg.agent_id" class="flex justify-center my-4">
              <span class="bg-gray-100/80 dark:bg-gray-700/80 backdrop-blur-sm border border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-300 text-xs px-3 py-1.5 rounded-full shadow-sm font-medium">
                {{ msg.content }}
              </span>
            </div>
            ...
            <div v-else class="flex space-x-3 max-w-[85%] md:max-w-[75%]" :class="msg.role === 'user' ? 'ml-auto justify-end' : ''">
              ...
              <div class="flex-shrink-0 flex flex-col justify-end" v-if="msg.role !== 'user'">
                <div class="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold text-xs shadow-sm ring-2 ring-white dark:ring-gray-600">
                ...
                <div 
                  ...
                  :class="[
                    msg.role === 'user' 
                      ...
                      : 'bg-[#F2F2F7] dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-2xl rounded-tl-sm'
                  ]"
...
const finishChat = async () => {
  if (!room.value) return
  if (!confirm('Are you sure you want to terminate this conversation permanently?')) return
  try {
    finishing.value = true
    await roomApi.finish(roomId)
