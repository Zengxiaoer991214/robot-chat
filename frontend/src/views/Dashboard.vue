<template>
  <div class="px-4 sm:px-0">
    ...
    <div v-else-if="rooms.length === 0" class="text-center py-20 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
      <div class="mx-auto h-12 w-12 text-gray-400 mb-4">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" />
        </svg>
      </div>
      <h3 class="text-lg font-medium text-gray-900 dark:text-white">No rooms yet</h3>
      <p class="mt-1 text-gray-500 dark:text-gray-400">Get started by creating your first chat room.</p>
    </div>
    ...
    <div v-else class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <div
        v-for="room in rooms"
        :key="room.id"
        class="group relative bg-white/70 dark:bg-gray-800/80 backdrop-blur-md border border-white/20 dark:border-gray-700 rounded-2xl p-6 shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer hover:-translate-y-1"
      ...
      >
        ...
        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2 line-clamp-1 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{{ room.name }}</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4 line-clamp-2 min-h-[2.5rem]">{{ room.topic }}</p>
        
        <div class="flex items-center justify-between pt-4 border-t border-gray-100/50 dark:border-gray-700/50">
